...
;
(function () {
    if (window.MFWMA) { //防止多次执行
        return;
    }
    //链接处理工具集
    var URLUtils = {
        parseUrl: function (url) {
            var parts, hash, queryStr, uri, mainHost;
            url = url || '';
            parts = url.split('#');
            hash = parts[1];
            parts = parts[0].split('?');
            uri = parts[0];
            queryStr = parts[1];
            mainHost = (uri.split('/')[2] || '').split('.').slice(-2).join('.');
            return {
                hash: hash,
                uri: uri,
                queryStr: queryStr,
        ...
        getParameter: function (queryStr, k) {
            ...
            matches = queryStr.match(reg);
            if (matches) {
                match = matches[matches.length - 1];
                v = match.substring(match[0] == '&' ? k.length + 2 : k.length + 1);
            }
            return v;
        },
        lazyJump: function (url) {
            if (url != location.href) {
                if (typeof history.replaceState == 'function') {
                    debugLog('replace state', url);
                    history.replaceState(null, null, url);
                } else {
                    debugLog('replace url', url);
                    location.replace(url);
                }
            }
        },
        //同步参数，实现了同步多个参数的功能，目前仅用于同步小程序参数
        syncParameter: function (url, referUrl, keys, checkValid) {
            var referObj = URLUtils.parseUrl(referUrl),
                obj = URLUtils.parseUrl(url),
                k,
                v,
                referV;
            checkValid = typeof checkValid == 'function' ? checkValid : function () {
            };
            for (var i = 0; i < keys.length, k = keys[i]; i++) {
                referV = URLUtils.getParameter(referObj.queryStr, k);
                if (referV && checkValid(k, referV, obj, referObj) !== false) {
                    v = URLUtils.getParameter(obj.queryStr, k);
                    if (!v) {
                        obj.queryStr = obj.queryStr || '';
                        if (v == undefined) {
                            obj.queryStr += (obj.queryStr ? '&' : '') + k + '=' + referV;
                        } else {
                            obj.queryStr = obj.queryStr.replace(k + "=" + v, k + "=" + referV);
                        }
                        url = obj.uri + '?' + obj.queryStr + (obj.hash !== undefined ? '#' + obj.hash : '');
                    }
                }
            }
            return url;
        ...
        typeof console == 'object' && console.log.apply(null, arguments);
    };
    var env = {},
        ua = navigator.userAgent;
    if (ua.match(/miniprogram/i) || window.__wxjs_environment === 'miniprogram') {
        env.platform = 'wx';
    } else if (ua.match(/swan\//i)) {
        env.platform = 'swan';
    }
    if (env.platform) {
        try {
            //根据referrer设置参数
            var url = URLUtils.syncParameter(location.href, document.referrer, ['_sFromMA'], function (k, v, obj, referObj) {
                if (obj.mainHost !== referObj.mainHost) { //域名不同
                    return false;
                }
            });
            //跳转带参数链接
            URLUtils.lazyJump(url);
            //小程序环境参数
            var fromMA = URLUtils.getParameter(location.search, '_sFromMA');
            if (fromMA && (fromMA = fromMA.split('_')) && fromMA[0] == env.platform) {
                env.appid = fromMA[1];
            }
            //点击链接添加参数 这样可以尽可能的保证参数传递下去
            if (env.appid) {
                document.addEventListener("click", function (e) {
                    var t = e.target, a, href, newHref;
                    while (t) {
                    ...
                    if (a && (href = a.getAttribute("href"))) {
                        if (href.indexOf('#') !== 0 && href.indexOf('javascript:') !== 0) { //锚点及JS不处理
                            newHref = URLUtils.syncParameter(href, location.href, ['_sFromMA'],
                                function (k, v, obj, referObj) {
                                    if (obj.mainHost && obj.mainHost !== referObj.mainHost) { //域名不同
                                        return false;
                                    }
                                });
                            if (newHref != href) {
                                debugLog(href, newHref);
                                a.setAttribute('href', newHref);
                                !a.getAttribute("_ma_init_href") && a.setAttribute("_ma_init_href", href);
                            }
                        }
                    }
                }, false);
