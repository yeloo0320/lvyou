...
$(function () {
    try {
        var cookieName = 'remember_search_history_in_cookie';

        function clearSearchHistoryCookie() {
            if ($.xcookie && $.xcookie.deleteCookie) {
                $.xcookie.deleteCookie(cookieName, {path: '/', expires: 10, domain: 'lvmama.com'});
            }
        }
    ...
    var $lv_city_box = $('.lv_city_box'), $search_city = $('.search_city'), $search_type = $(".search_type");
    $(document).bind("click", function () {
        ...
        $.searchComplete_hotelAddon && $.searchComplete_hotelAddon.autoSelectFirst && $.searchComplete_hotelAddon.autoSelectFirst();
        $.hotelComplete && $.hotelComplete.autoSelectFirst && $.hotelComplete.autoSelectFirst()
    });
    ...
    function searchType() {
        ...
        if ($('body').hasClass('around') || $('body').hasClass('zijia')) {
            ...
            typeNum = 5;
        } else if ($('body').hasClass('hotel') || $('body').attr('data-search-type') === "HOTEL") {
            typeNum = 6;
        ...
        var typeIndex = $search_type_tab.find('li').eq(typeNum), typeText = typeIndex.text(),
        ...
        $search_type_tab.parents(".lv_search_box").first().attr("data-search-type", typeData)
    ...
    $('.search_type_tab li').click(function () {
        var $this = $(this);
        var typeText = $this.text(), typeCode = $this.attr("data-type"), num = $this.index();
        ...
        $this.parents(".lv_search_box").first().attr("data-search-type", typeCode)
    });
    ...
    $(".js_cf_city,.lv_s_city").bind("click", function (e) {
        ...
        if (target.tagName.toLowerCase() === "a") {
            ...
            var dataid = $target.attr("data-id"), datacode = $target.attr("data-code");
    ...
    var $btn_lv_search = $('.btn_lv_search');
        ...
        "click": function (e) {
            ...
            if (target.tagName.toLowerCase() == "a") {
                ...
                if ($('body').hasClass('home')) {
                    ...
                    $("input:text[active]").val(text).removeClass("des_tip");
                    $btn_lv_search.attr("data-dropdownlist", "true");
                    $btn_lv_search.trigger('click');
                }
                $(this).hide();
            }
        }, "mousedown": function (e) {
    ...
    var oldValue = ['请输入目的地、主题、或关键词', '请输入目的地、主题、或关键词', '请输入目的地、主题、或关键词', '请输入目的地、主题、或关键词', '请输入目的地、主题、或关键词', '请输入目的地、主题或景区名称', '请输入航线、邮轮公司或邮轮名称', '请输入您需要签证的国家/地区', '请输入目的地、主题、或关键词'];
    ...
    $(".btn_lv_search").bind("click", function (e) {
        ...
        if (type.toLowerCase() == "hotel") {
            var $startDate = (".search_type_list:visible #startDate");
            var $endDate = $(".search_type_list:visible #endDate");
            if ($startDate.length && $endDate.length) {
                ...
                var start_Date = startDate.substring(0, 4) + "-"
                    + startDate.substring(4, 6) + "-" + startDate.substring(6);
                var end_Date = endDate.substring(0, 4) + "-" + endDate.substring(4, 6)
                    + "-" + endDate.substring(6);
                document.cookie = "startDate_cookie =" + start_Date + ";" + expires
                    + ";path=/" + ";domain=.lvmama.com";
                document.cookie = "endDate_cookie =" + end_Date + ";" + expires
                    + ";path=/" + ";domain=.lvmama.com";
            }
            e.stopPropagation();
            $.searchComplete_hotelAddon.searchAndJump({startDateStr: start_Date, endDateStr: end_Date});
        } else {
            searchStatistics();
...
(function ($) {
    $.searchType = {
        showType: function (type) {
            ...
            var $search_type_tab = $('.search_type_tab'), typeIndex = $search_type_tab.find('li').eq(typeNum),
                typeText = typeIndex.text(), typeData = typeIndex.attr('data-type');
            typeIndex.hide().siblings().show();
            $('.search_type_list').eq(typeNum).show().siblings().hide();
            $('.js_btn_type').find('b').text(typeText).attr('data-type', typeData);
            $search_type_tab.parents(".lv_search_box").first().attr("data-search-type", typeData)
...
$(function () {
    ...
    $("div.Js_LISTFIRST a").live('click', function (e) {
        e.stopPropagation();
        var $body = $('body'), $this = $(this), citypinyin = $this.attr('pinyin');
        ...
        if ($body.hasClass('ticket_city')) {
            ...
            $.xcookie.setCookie('_ticket_ip_city_place_id', cityId, {path: '/', expires: 10, domain: 'lvmama.com'});
            $.xcookie.setCookie('_ticket_ip_city_name', zhandianName, {path: '/', expires: 10, domain: 'lvmama.com'});
        } else if (!/dujiaseolist/.test($body.attr('id'))) {
            $.xcookie.setCookie('_ip_province_place_id', provinceId, {path: '/', expires: 10, domain: 'lvmama.com'});
            $.xcookie.setCookie('_ip_city_place_id', cityId, {path: '/', expires: 10, domain: 'lvmama.com'});
            $.xcookie.setCookie('_ip_city_name', zhandianName, {path: '/', expires: 10, domain: 'lvmama.com'});
        }
        ;
        if (!$body.hasClass('search_list') && !$body.hasClass('d_list')) {
            var _form = $("<form method=\"post\" >" + "<input type=\"hidden\" name=\"provinceId\" value=\"" + provinceId + "\" />" + "<input type=\"hidden\" name=\"cityId\" value=\"" + cityId + "\" />" + "<input type=\"hidden\" name=\"stationName\" value=\"" + zhandianName + "\" /></form>");
            $box.prepend(_form);
            _form.submit();
        } else {
            var url = '';
    ...
    function dujiaUrl(dataid) {
        ...
        for (var i = 0; i < typeOfSeo.length; i++) {
            ...
            if (url.indexOf(thisType) >= 0) {
                var parrten = new RegExp('(' + thisType + ')([^\?]+)'), urltext = url.replace(parrten, '$1');
    ...
    function rebuildUrl(placeid) {
        var tempAnchor = null, tempParam = null, partten = null;
    ...
    function showPnavDown() {
        ...
        /hotel/.test($bodyClass) && (!/search_list/.test($bodyClass)) && pnavDown($('#hotel'));
    ...
    function pnavDown(nav) {
        ...
        if (nav.length != 0) {
            var ThisL = nav.offset().left, navL = $('.lv_nav').offset().left, navW = $('.lv_nav').width(),
                winW = $('body').width(), pW = nav.find('p').width() / 2, thisW = nav.width() / 2,
                downL = ThisL + thisW;
            nav.find('.down_nav_t').css('left', downL - 5);
            if (navL > downL - pW) {
                downL = navL + pW
            } else if (navL + navW < ThisL + pW + thisW) {
    ...
    if ($('#goTopBtn').length == 0) {
        ...
        if (!$body.hasClass('home')) {
            ...
            if ($body.hasClass('ticketChannel')) {
                $("#right-bottom-tools").append('<span id="rbtApplet" class="JS_appletPop" tip-content="<img src=\'http://pic.lvmama.com/img/v6/applet_poptip_ticket.png\' width=\'160\' height=\'184\'>"></span>');
            }
            if ($body.hasClass('hotel')) {
                $("#right-bottom-tools").append('<span id="rbtApplet" class="JS_appletPop" tip-content="<img src=\'http://pic.lvmama.com/img/v6/applet_poptip_hotel.png\' width=\'160\' height=\'184\'>"></span>');
            }
            var gotop = true;
    ...
    function xhshowtips() {
        $('.tapbar-inner').append('<span id="stips" class="stips"><i class="sarrow"></i>验证邮箱可获300积分，<a href="http://my.lvmama.com/myspace/userinfo.do" hidefocus="false">立即验证</a><i class="sclose">×</i></span>');
        var useremail = "";
        try {
    ...
    $.xcookie = {
        cookie: function (name, value, options) {
            var path = options.path ? '; path=' + (options.path) : '',
                ...
                secure = options.secure ? '; secure' : '', expires = '';
            ...
            var cookieValue = null, doc = document;
...
$(function () {
    ...
    var loadImg = function () {
        var This = $(this);
        if (!This.is(':hidden')) {
            var to_sc = This.attr('to_sc'), js_sc = This.attr('js_sc'), to = This.attr('to');
            if (to_sc) {
...
(function () {
        ...
        if (!$.xcookie.getCookie(footAppCook)) {
            setTimeout(function () {
                ...
                $('.footBar-close').click(function () {
                    ...
                    $.xcookie.setCookie(footAppCook, footAppCook, {path: '/', expires: 30, domain: 'lvmama.com'});
                });
            }, 5000);
        }
    }
    ...
    function syFootApp() {
        ...
        setTimeout(function () {
            ...
            $('.footBar-close').click(function () {
                $footBar.stop(true, true).animate({left: "-100%"}, 1000, function () {
                    $footBarOpen.stop(true, true).animate({left: "0"}, 300);
                });
                $.xcookie.setCookie(footAppCook, footAppCook, {path: '/', expires: 7});
            });
            $('.syfootBarOpen').click(function () {
                $footBarOpen.stop(true, true).animate({left: "-123px"}, 300, function () {
                    $footBar.stop(true, true).animate({left: "0"}, 1000);
                });
                $.xcookie.deleteCookie(footAppCook, footAppCook, {path: '/'});
            });
            if (!$.xcookie.getCookie(footAppCook)) {
                $footBar.css({"left": 0});
            } else {
                $footBarOpen.css({"left": 0});
            }
        }, 5000);
    }
})();
$(function () {
    ...
    $document.on("click", ".lv_top_promotion_close", function () {
        $.xcookie.setCookie(lv_top_promotion_cookie_name, 'yes', {path: '/', expires: 7});
...
$(function () {
    ...
    $('.lv_nav_l').on('click', 'a', function (ev) {
        var This = $(this), losc = This.attr('data-losc');
        if (losc) {
            ev.preventDefault();
...
$(function () {
    ...
    function channelFloat() {
        ...
        var $channel_float_l = $('.channel_float_l'), $channel_float_b = $('.channel_float_b'),
        ...
        function mouseout() {
            $channel_float_s.stop(true).animate({'width': 0}, 400, function () {
        ...
        function mousein() {
            ...
            $channel_float_s.show().stop(true).animate({'width': 167}, 400);
        };
    }
});
(function ($) {
    $.fn.extend({
        focusBanner: function (options) {
            ...
            return this.each(function () {
                var o = options, $me = $(this), yjzfirst = !o.tosrc ? "src" : "to", yjz = !o.tosrc ? "src" : "js_to";
                $.ajax({
                    url: o.url,
                    type: 'GET',
                    dataType: "jsonp",
...
;(function ($) {
    $.fn.poptip = function (options) {
        ...
        $(this)[opt.bindevent](opt.trigger, function () {
            ...
            clearTimeout(triggerObj.timeId);
            obj.hide();
            if (title) {
            ...
            if (opt.skin != 'default') {
                $("#poptip" + opt.templete).attr('class', 'poptip tip-light poptip-' + opt.skin);
            } else {
                $("#poptip" + opt.templete).attr('class', 'poptip tip-light poptip-default');
            }
            if (content) {
            ...
            triggerObj.timeId = setTimeout(function () {
                $("#poptip" + opt.templete).hide();
            }, opt.hovershow);
            $("#poptip" + opt.templete).data("timeId", triggerObj.timeId);
        });
